...
class IntegrateGradExplainer(VanillaGradExplainer):
    ...
    def explain(self, inp, ind):
        ...
        for alpha in np.linspace(0, 1.0, num=self.steps):
            ...
            grad += g
        print("EXPLANATION")
        print(torch.sum(grad * inp_data / self.steps, dim=2))
        print("PREDICTION")
        print(torch.gather(torch.softmax(self.model(**new_inp)[0], dim=1), 1, ind.unsqueeze(0).t()))
        print("ZERO PREDICTION")
        new_inp[self.input_key] = inp_data * 0
        print(torch.gather(torch.softmax(self.model(**new_inp)[0], dim=1), 1, ind.unsqueeze(0).t()))
